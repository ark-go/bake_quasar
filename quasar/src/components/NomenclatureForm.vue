...
<script>
import { ref, toRefs, toRef, watch, onMounted, computed } from "vue";
...
export default {
  ...
  props: ["dataRow", "spravAll", "resultCheck"],
  setup(props, { emit }) {
    ...
    watch(formData, () => {
      emit("changeFormData", formData);
    });
    const { resultCheck } = toRefs(props); // десруктуризация с соххранеием реактивности
    watch(resultCheck, () => {
      resultatCheck(resultCheck.value);
    });
    // const dataForm = ref({});
    ...
    async function onClick() {
      ...
      if (isNewUser.value) {
        ...
        // console.log("addData: data:", data, "formData:", formData.value);
