 ...
 * @returns
 */
export async function kagentBakeryOwn(req, res) {
  let timezone = req?.headers?.timezone
    ? req.headers.timezone
    : "Europe/Moscow";

  let mess = "Таблица территории пекарни, cmd: " + req.body?.cmd;
  ...
  switch (req.body.cmd) {
    ...
    case "addToGroup":
      ...
      return await addToGroup(req, res, "kagent_x_bakery_own", timezone);
    case "moveToGroup":
      // Перенос территорию к печке
      return await addToGroup(req, res, "kagent_x_bakery_own", timezone);
    case "info":
      // Перенос территорию к печке
      return await info(req, res, "kagent_x_bakery_own", timezone);
    case "removeFromGroup":
      //   // Перенос территорию к печке
      return await removeFromGroup(req, res, "kagent_x_bakery_own", timezone);
