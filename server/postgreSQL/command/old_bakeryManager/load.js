...
export async function load(req, res, tabname, timezone, idOne) {
  ...
  if (req.body?.territory_id) {
    if (req.body.nogroup) {
      // не в группе
      wher = /*sql*/ `WHERE tr.territory_id <> ${req.body.territory_id}`;
    } else {
      // в других группах
      wher = /*sql*/ `WHERE tr.territory_id = ${req.body.territory_id}`;
    }
  ...
  if (req.body.free) {
    ...
    wher = /*sql*/ `WHERE tr.territory_id IS NULL`;
  }
  ...
  let sqlP = {
    ...
    -- (select count(*) from bakery_manager where user_id = users.id AND is_last = true ) as bakery_count,
      ...
      ${tabname}.meta
      FROM ${tabname}
      LEFT JOIN  ( select * from territory_manager 
        where  is_last = true ) as tr  ON tr.user_id = ${tabname}.id  -- user
      ${wher}
