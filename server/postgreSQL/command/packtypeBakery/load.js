...
export async function load(req, res, tabname, timezone, idOne) {
  ...
  if (req.body?.territory_id) {
    if (req.body.nogroup) {
      // не в группе
      wher = /*sql*/ `WHERE packtype_g.id <> ${req.body.territory_id}`;
    } else {
      // в других группах
      wher = /*sql*/ `WHERE packtype_g.id = ${req.body.territory_id}`;
    }
  ...
  if (req.body.free) {
    ...
    wher = /*sql*/ `WHERE packtype_g.id IS NULL`;
  }
  console.log("packtypeBakery", tabname, req.body);
  ...
  let sqlP = {
    ...
    -- (select count(*) from bakery_manager where user_id = users.id AND is_last = true ) as bakery_count,
      ...
      ${tabname}.meta
      FROM ${tabname}
          LEFT JOIN  ( select * from packtype_x_bakery  -- territory-parent / bakery- child
                      where  is_last = true ) as pack  ON pack.child_id = ${tabname}.id --  пекарни по child
          LEFT JOIN  packtype as packtype_g ON packtype_g.id = pack.parent_id -- территории по parent
      -- мнеджер печки
