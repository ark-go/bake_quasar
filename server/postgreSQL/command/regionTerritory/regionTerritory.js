 ...
 * @returns
 */
export async function regionTerritory(req, res) {
  let timezone = req?.headers?.timezone
    ? req.headers.timezone
    : "Europe/Moscow";

  let mess = "Таблица регионы территории, cmd: " + req.body?.cmd;
  ...
  switch (req.body.cmd) {
    ...
    case "addToGroup":
      ...
      return await addToGroup(req, res, "region_territory", timezone);
    case "moveToGroup":
      // Перенос территорию к печке
      return await addToGroup(req, res, "region_territory", timezone);
    case "info":
      // Перенос территорию к печке
      return await info(req, res, "region_territory", timezone);
    case "removeFromGroup":
      //   // Перенос территорию к печке
      return await removeFromGroup(req, res, "region_territory", timezone);
