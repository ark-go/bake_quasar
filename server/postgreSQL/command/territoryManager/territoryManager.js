 ...
 * @returns
 */
export async function territoryManager(req, res) {
  let timezone = req?.headers?.timezone
    ? req.headers.timezone
    : "Europe/Moscow";

  let mess = "Таблица терр менеджеры, cmd: " + req.body?.cmd;
  ...
  switch (req.body.cmd) {
    ...
    case "addToGroup":
      ...
      return await addToGroup(req, res, "territory_manager", timezone);
    case "moveToGroup":
      // Перенос территорию к печке
      return await addToGroup(req, res, "territory_manager", timezone);
    case "info":
      // Перенос территорию к печке
      return await info(req, res, "territory_manager", timezone);
    case "removeFromGroup":
      //   // Перенос территорию к печке
      return await removeFromGroup(req, res, "territory_manager", timezone);
