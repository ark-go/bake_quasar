...
export async function load(req, res, tabname, timezone, idOne) {
  ...
  if (req.body?.territory_id) {
    if (req.body.nogroup) {
      // не в группе
      wher = /*sql*/ `WHERE trademark_g.id <> ${req.body.territory_id}`;
    } else {
      // в других группах
      wher = /*sql*/ `WHERE trademark_g.id = ${req.body.territory_id}`;
    }
  ...
  if (req.body.free) {
    ...
    wher = /*sql*/ `WHERE trademark_g.id IS NULL`;
  }
  ...
  let sqlP = {
    ...
    --  region_g.name as region_name,
      to_char(tr.date_start at time zone $1,  'DD.MM.YYYY') as "date_start_trademark",
    ...
    -- (select count(*) from bakery_manager where user_id = users.id AND is_last = true ) as bakery_count,
      ...
      ${tabname}.meta
      FROM ${tabname}
          LEFT JOIN  ( select * from trademark_x_bakery  -- territory-parent / bakery- child
                      where  is_last = true ) as tr  ON tr.child_id = ${tabname}.id --  
          LEFT JOIN  trademark as trademark_g ON trademark_g.id = tr.parent_id -- 
      -- мнеджер печки
