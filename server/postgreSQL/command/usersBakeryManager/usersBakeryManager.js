 ...
 * @returns
 */
export async function usersBakeryManager(req, res) {
  let timezone = req?.headers?.timezone
    ? req.headers.timezone
    : "Europe/Moscow";

  let mess = "Таблица юзер Пекарни, cmd: " + req.body?.cmd;
  ...
  switch (req.body.cmd) {
    ...
    case "addToGroup":
      ...
      return await addToGroup(req, res, "users_x_bakery_manager", timezone);
    case "moveToGroup":
      // Перенос территорию к печке
      return await addToGroup(req, res, "users_x_bakery_manager", timezone);
    case "info":
      // Перенос территорию к печке
      return await info(req, res, "users_x_bakery_manager", timezone);
    case "removeFromGroup":
      //   // Перенос территорию к печке
      return await removeFromGroup(
        req,
        res,
        "users_x_bakery_manager",
        timezone
      );
