...
--      article CITEXT UNIQUE DEFAULT nextval('products_raw_sequence')::CITEXT NOT NULL ,
      ...
      FOREIGN KEY (producttype_id) REFERENCES producttype ("id") ON DELETE RESTRICT
    );

COMMENT ON TABLE productvid IS 'Вид продукции.';
--!!! Будет fullname  как дополнительное имя nameext
ALTER TABLE productvid ADD COLUMN nameext CITEXT;
alter table productvid add constraint nameext CHECK(char_length(trim(nameext)) <> 0) ;  -- enable novalidate не будет проверять таблицу
...
DROP TABLE IF EXISTS productraw CASCADE;
    CREATE TABLE IF NOT EXISTS productraw (
      ...
      productrawvid_id BIGINT NOT NULL,  -- вид сырья
      name CITEXT UNIQUE NOT NULL,       -- наименование
      fullname CITEXT DEFAULT '', -- полное имя
      unit_id BIGINT NOT NULL,  -- ед измерения
      
      description CITEXT default '', -- просто так
      article_buh CITEXT UNIQUE,   -- если захотят интеграцию
      article CITEXT UNIQUE DEFAULT nextval('products_raw_sequence')::CITEXT NOT NULL ,  -- наш собственный артикле
      -- -- -- --
      user_id BIGINT NOT NULL DEFAULT 0,
      user_date TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
      meta jsonb NOT NULL default '{}'::jsonb,
...
CREATE OR REPLACE FUNCTION f_productraw_primary_key()
  RETURNS trigger AS
$func$
BEGIN
...
CREATE TRIGGER productraw_primary_key
  BEFORE INSERT ON productraw  --
  FOR EACH ROW
  EXECUTE PROCEDURE f_productraw_primary_key();
-- --------- BEFORE UPDATE Primary key
CREATE OR REPLACE FUNCTION f_productraw_upd()
  RETURNS trigger AS
$func$
BEGIN
   NEW.is_raw := TRUE;
...
CREATE TRIGGER productraw_upd
  BEFORE UPDATE ON productraw  -- 
  FOR EACH ROW
  EXECUTE PROCEDURE f_productraw_upd();
-- --------- продукция продается
...
DROP TABLE IF EXISTS products CASCADE;
    CREATE TABLE IF NOT EXISTS products (
      ...
      name CITEXT NOT NULL,       --  короткое имя куска от окончания названия - вида продукта
