...
CREATE TABLE IF NOT EXISTS users_permiss_all (
    ...
    code_name CITEXT UNIQUE NOT NULL CHECK(code_name ~ '^[A-Za-z][A-Za-z1-9_]{3,19}$'), -- 4-20 лат букв и подчерк
...
CREATE TABLE IF NOT EXISTS users_group_tree (
    id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    parent_id BIGINT NOT NULL CHECK(parent_id <> id) REFERENCES users_group_permiss (id) ON DELETE CASCADE, -- дерево собственное
    group_name CITEXT UNIQUE NOT NULL CHECK(char_length(trim(code_namme)) >= 4),
    group_code UNIQUE CITEXT NOT NULL CHECK(char_length(trim(code_namme)) >= 4),
    description CITEXT NOT NULL CHECK(char_length(trim(description)) >= 10),
    meta jsonb default '{}'
)
